...
const CheckCibilCard = ({ cardData, pathName, position, title }) => {
  ...
  return (
    <div
      ...
      <div className={`${cardData?.eligibile ? 'mt-[22px] mb-[18px]' : 'mt-[40px] mb-[30px]'}`}>
        <Image
            src={cardData?.image}
      ...
      <div className='flex flex-col gap-[2px] px-[10px] max-sm:px-[20px]'>
        ...
        <div className='mt-[24px] mb-[40px] text-center' onClick={() => sendEventToGTM(getPromotionObject(eventData))}>
          <Link href={pathName ? pathName : cardData?.pathName} prefetch={false}>
