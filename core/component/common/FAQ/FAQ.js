...
const FAQ = React.memo(({ faqdata, SelectSupportTabs }) => {
  ...
  // Function to generate FAQ JSON-LD schema
  const addFaqJsonLd = useCallback(() => {
    const faqJsonLd = {
    ...
    if (faqdata && faqdata.question_answer?.length > 0) {
      faqdata.question_answer.forEach((faqItem) => {
