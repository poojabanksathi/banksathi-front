...
const CreditBeginnerCard = React.memo(({ longTerm }) => {
  // Memoize the HTML content if longTerm changes
  const contentHtml = React.useMemo(() => {
    return `<div>${longTerm?.long_form_content?.long_form_content || longTerm?.long_form_content}</div>`;
