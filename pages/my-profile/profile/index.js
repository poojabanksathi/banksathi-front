...
export async function getServerSideProps(context) {
  try {
    ...
    const response5 = await Axios.post(BASE_URL + COMMON?.metaDetailPage, metaParams).catch((error) => {
      return null
    })
    const [data1, data2, data4] = await Promise.all([response1, response2, response5]).then(
